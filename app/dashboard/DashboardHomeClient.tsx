...
import { Eye, EyeOff, Bot, ClipboardList, TrendingUp, TrendingDown, ArrowRight, Plus, Activity } from "lucide-react";
...
export default function DashboardHomeClient({ displayName, planLabel, activeRobotCount, robots }: Props) {
            ...
            {robots.map((r) => (
              <div key={r.robotId} className={styles.tickerCard}>
                <div className={styles.tickerInfo}>
                  <div className={styles.tickerIcon} style={{ background: (r.meta?.color ?? "var(--accent-primary)") + "22" }}>
                    <Activity size={20} color={r.meta?.color ?? "var(--accent-primary)"} />
                  </div>
                  <div>
                    <div className={styles.tickerName}>{r.meta?.name ?? r.robotId}</div>
                    <div className={styles.tickerPrice} style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                      {r.meta?.tagline}
                    </div>
                  </div>
                </div>
                <div className={styles.tickerBadge} style={{ background: "rgba(16,185,129,0.1)", color: "var(--accent-primary)", border: "1px solid rgba(16,185,129,0.2)" }}>
